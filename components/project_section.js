    ...
    <Link href={`/work/${project.slug}`} className="group block relative">
      <div className="relative grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-10 items-center py-10 md:py-16 border-b border-text/10">
        ...
        <div className={"relative z-10 md:col-span-6 h-56 sm:h-72 md:h-96 rounded-2xl overflow-hidden " + (reversed ? "md:order-2" : "")}>
          <div className="absolute inset-0 bg-gradient-to-t from-surface-950/60 to-transparent z-10" />
        ...
        <div className={"relative z-10 md:col-span-6 space-y-4 " + (reversed ? "md:order-1" : "")}>
          <div className="flex flex-wrap gap-2">
            {project.tags.slice(0, 3).map((tag, i) => (
              <span key={i} className="px-2 py-1 text-xs rounded-full bg-surface-900/80 text-primary border border-primary/20">{tag}</span>
            ))}
          </div>
          ...
          <p className="text-surface-400 text-sm max-w-md">{project.description}</p>
  ...
  return (
    <section id="projects" className="mt-20 px-4 sm:px-10 md:px-20 lg:px-40">
      <div className="flex flex-col items-center text-center max-w-2xl mx-auto">
        ...
        <p className="mt-6 text-surface-400 max-w-2xl">Real problems, real fixes, real numbers. Open a file for the full breakdown.</p>
      </div>
